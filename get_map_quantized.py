...
if __name__ == "__main__":
    ...
    #   默认指向根目录下的VOC数据集
    #-------------------------------------------------------#
    VOCdevkit_path  = '../dataset/aluminum_VOC'
    #-------------------------------------------------------#
    #   结果输出的文件夹,默认为map_out
    #-------------------------------------------------------#
    map_out_path    = 'map_out'

    image_ids = open(os.path.join(VOCdevkit_path, "train/ImageSets/Main/test.txt")).read().strip().split()
    ...
    if map_mode == 0 or map_mode == 1:
        ...
        # ssd = torch.load(QAT_save_path)
        print("Load model done.")

        print("Get predict result.")
        for image_id in tqdm(image_ids):
            image_path  = os.path.join(VOCdevkit_path, "train/JPEGImages/"+image_id+".jpg")
    ...
    if map_mode == 0 or map_mode == 2:
        ...
        for image_id in tqdm(image_ids):
            with open(os.path.join(map_out_path, "ground-truth/"+image_id+".txt"), "w") as new_f:
                root = ET.parse(os.path.join(VOCdevkit_path, "train/Annotations/"+image_id+".xml")).getroot()
